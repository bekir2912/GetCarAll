...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $new_shops = Shop::find()->where(['>=', 'created_at', strtotime(date('Y-m-d 00:00:00'))])->count();
        $all_shops = Shop::find()->count();
        $success_shops = Shop::find()->where(['status' => 1])->count();

        $users_today = User::find()->where(['>=', 'created_at', strtotime(date('Y-m-d 00:00:00'))])->count();
        $users = User::find()->count();
        $users_active = User::find()->where(['status' => 10])->count();

        $products_today = Product::find()->where(['>=', 'created_at', strtotime(date('Y-m-d 00:00:00'))])->count();
        $products = Product::find()->count();
        $products_active = Product::find()->where(['status' => 1])->count();
        ...
        foreach ($categories as $category) {
            ...
            foreach ($dates as $date) {
                $products_by_cat[$category->id]['dates'][$date] = Product::find()
        ...
        $all_products = [];
        foreach ($dates as $date) {
            $all_products['dates'][$date] = Product::find()
                ->andWhere(['>=', 'created_at', strtotime($date.' 00:00:00')])
                ->andWhere(['<=', 'created_at', strtotime($date.' 23:59:59')])
                ->count();
        }
        ...
        $shops = Shop::find()->orderBy('`id` DESC')->limit(5)->all();
