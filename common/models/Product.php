...
class Product extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['add_cats', 'add_cats_titles', 'currency', 'type', 'vin', 'auto_number'], 'string'],
            [['articul'], 'string'],
            [['price_usd', 'deposit'], 'number', 'min' => 0],
            [['price', 'km', 'is_deposit', 'unlimited_mileage'], 'safe'],
            [['from', 'phone_views'], 'integer','min' => 0],
            [['year'], 'integer', 'max' => date('Y'), 'min' => 1900],
            [['date_off'], 'date'],
            [['wholesale', 'in_order', 'excess_mileage', 'custom_options'], 'safe'],
            [['wholesale_unit_id', 'is_moderated', 'label'], 'safe'],
            [['min_experience','min_age', 'min_day','excess_time', 'included_kilometers', 'shop_id', 'category_id', 'brand_id','user_id', 'lineup_id',  'price_type', 'unit_id', 'sale_id', 'status', 'warranty',  'interesting',  'interesting_order',  'view', 'created_at', 'updated_at'], 'integer'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'user_id' => 'Пол-ль',
            'unit_id' => 'за...',
            'label' => Yii::t('frontend', 'Название авто'),
            'type' => Yii::t('frontend', 'Тип авто'),
            ...
            'auto_number' => Yii::t('frontend', 'Гос. номер авто'),
            'date_off' => Yii::t('frontend', 'Date Off'),
            'is_deposit' => Yii::t('frontend', 'Без депозита'),
            'deposit' => Yii::t('frontend', 'Депозит'),
            'unlimited_mileage' => Yii::t('frontend', 'Неограниченный пробег'),
            'included_kilometers' => Yii::t('frontend', 'Включено километров (км в сутки)'),
            'excess_time' => Yii::t('frontend', 'Доплата за превышение времени (в день)'),
            'excess_mileage' => Yii::t('frontend', 'Доплата за превышение пробега (за км)'),
            'min_experience' => Yii::t('frontend', 'Минимальный необходимый стаж вождения'),
            'min_age' => Yii::t('frontend', 'Минимальный возраст водителя'),
            'min_day' => Yii::t('frontend', 'Минимальные дни аренды'),
            'vin' => Yii::t('frontend', 'Vin'),
...
//            'price' => Yii::t('frontend', 'Product Price'),
    ...
    {
        return $this->hasOne(ProductStatus::className(), ['id' => 'status']);
    }
}
