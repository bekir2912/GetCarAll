                ...
                <div class="service__link-block clearfix">
                    <?php for ($s = 0; $s < count($category->activeCategories); $s++) { ?>
                        <a class="service-block__link" href="<?=Url::to(['service/list', 'id' => $category->activeCategories[$s]->url])?>">
                            <img src="<?=$category->activeCategories[$s]->icon?>" alt="" style="width: 20px;display: inline-block;vertical-align: top"/>
    ...
    <div class="map-container">
        ...
        <script type="text/javascript">
            ...
            function init(){
                ...
                var myPlacemark<?=$i?> = new ymaps.Placemark([<?=$filtered_shops[$i]->info->lat?>, <?=$filtered_shops[$i]->info->lng?>], {
                    balloonContent: '<?=str_replace("'", '`', $filtered_shops[$i]->name)?>'
                }, {
